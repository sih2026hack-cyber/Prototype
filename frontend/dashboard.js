...
async function refreshDashboard(){
  ...
  try{
    ...
    $('#ingestStatus').textContent=mine&&['queued','running'].includes(mine.status)?(mine.status==='queued'?'Starting your link…':'Analysing your link: '+(status.job?.message||'working…')):status.busy?'Background refresh running: '+(status.job?.message||''):last?.status==='failed'?'Refresh failed · previous results retained':'Ready';
...
$('#sourceForm').addEventListener('submit',async e=>{e.preventDefault();try{const url=$('#sourceUrl').value.trim();const r=url?await post('/api/analysis/run',{url}):await post('/api/discovery/run',{});pendingRun=r.run_id;$('#ingestStatus').textContent=url?(r.queued?'Starting your link — background refresh stopped…':'Analysis started…'):r.accepted===false?(r.message||'Showing today’s trending analysis.'):'Finding today’s trending channels…';await refreshDashboard();}catch(e){$('#ingestStatus').textContent=e.message;}});
