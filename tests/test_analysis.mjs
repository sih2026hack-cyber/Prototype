...
const a=service.start({url:'https://youtu.be/'+video});assert.throws(()=>service.start({url:'https://youtu.be/'+video}),/already running/);await wait();assert.equal(service.view({run:a.run_id}).posts.length,1);assert.equal(service.view({run:'legacy'}).posts[0].id,records[0].id);
