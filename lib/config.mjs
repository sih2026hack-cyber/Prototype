...
// Repo copies of the team NLP module and moderator prompts are the defaults (used on Render).
env.ARGUS_NLP_ROOT ||= path.join(ROOT, 'nlp');
env.ARGUS_MODERATOR_ROOT ||= path.join(ROOT, 'vendor', 'ai-moderator');
const isPublic = Boolean(env.RENDER || env.ARGUS_PUBLIC === 'true');
export const server = {
  host: env.HOST || (isPublic ? '0.0.0.0' : '127.0.0.1'), isPublic,
  // Hostnames the app may be addressed by; Render provides RENDER_EXTERNAL_HOSTNAME automatically.
  hosts: [env.RENDER_EXTERNAL_HOSTNAME, ...(env.ARGUS_ALLOWED_HOSTS || '').split(',')].map(h => (h || '').trim().toLowerCase()).filter(Boolean),
  user: env.ARGUS_USER || 'argus', password: env.ARGUS_PASSWORD || '',
};
...
export const settings = {
  ...
  // Re-collect comments from today's trending selection this often (search runs only once a day).
  maxComments: Math.min(500, Math.max(10, Number(env.MAX_COMMENTS_PER_RUN || 80))), // prototype default: quick runs
  trendingRefreshMinutes: Math.max(15, Number(env.TRENDING_REFRESH_MINUTES || 60)), keepRefreshRuns: Math.max(4, Number(env.KEEP_REFRESH_RUNS || 24)),
