...
let log=null; // loaded on first use, after any boot-time state restore
export async function chat(question,history,posts,ingest,context={}) {
  ...
  log??=readLocal('chat-evidence',[]);log.unshift(saved);saveLocal('chat-evidence',log.slice(0,100));upsert('chat_evidence',[saved],'id').catch(()=>{});
