...
export function aggregate(posts) {
  ...
  for (const p of posts) {
    ...
    const topic = p.topic?.replace(/[·\s]/g,'') ? p.topic : p.topic_source==='BERTopic' ? `Topic ${p.topic_id+1} (keywords unavailable)` : 'Topic analysis pending';
    const key = p.topic_source==='BERTopic' ? `${p.source_group||'manual'}:${p.topic_id}` : topic;
