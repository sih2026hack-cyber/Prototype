...
export async function enrich(posts,progress,call=minimax,{names=new Map()}={}){
  ...
  const emotions=pool(batches,3,async batch=>{if(failures>=3)return;
...
export async function segmentPosts(posts,progress=()=>{},call=minimax,cache=null,names=new Map()){
  ...
  for(const p of comments){const hit=cache[segmentKey(p)]||(!names.has(p.id)&&cache[legacyKey(p)]);if(hit)p.segment=hit;}
