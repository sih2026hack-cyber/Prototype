...
export function insights(posts,run,filters={},audienceReport=null){
  const comments=posts.filter(p=>p.content_kind!=='video'),summary=aggregate(comments),topics=aggregate(posts).topics.filter(t=>t.source==='BERTopic'&&t.topic_id>=0),emotions={},days=new Map();let nuanced=0;
  for(const p of comments){
    ...
    if(p.topic_source==='BERTopic'&&p.topic_id>=0){const key=`${p.source_group||'manual'}:${p.topic_id}`;d.topics[key]=(d.topics[key]||0)+1;}days.set(date,d);
  }
  ...
  return {run,filters,summary,topics,timeline,emotions,nuanced,network:network(posts),audienceReport,participation:participation(comments),
    videos:posts.filter(p=>p.content_kind==='video').map(({embedding,nlp_row,author_ref,...p})=>p),
