...
import {worker,stopWorker} from './worker.mjs';
...
export function createAnalysisService(deps={}){
  ...
  // Every progress report is also a checkpoint where a pre-empted background refresh stops.
  const progress=(stage,message)=>{if(job?.cancelled)throw new ServiceError('Stopped so your link could run first.',499);if(job){job.stage=stage;job.message=message;job.updated_at=now();persist();}};
  ...
  async function execute(run,input){
    ...
    }catch(e){run.status=run.cancelled?'cancelled':'failed';run.stage=run.status;run.message=run.cancelled?'Stopped so a submitted link could run first; it refreshes again at its next scheduled time.':e.message;run.completed_at=now();}
  ...
  function start(input={}){
    ...
    if((busy||queue.length)&&mode!=='manual')throw new ServiceError('Another collection or sync is already running.',409);
    ...
    if(busy){
      // A user's link always wins: stop a background refresh (live/trending/discovery) right away.
      if(job&&job.mode!=='manual'&&!job.cancelled){job.cancelled=true;job.message='Stopping background refresh for your link…';(deps.stopWorker||stopWorker)();}
      if(queue.some(q=>q.input.url===input.url))return {accepted:true,queued:true,run_id:queue.find(q=>q.input.url===input.url).run.id};
      Object.assign(run,{status:'queued',stage:'queued',message:job?.cancelled?'Starting now…':'Waiting for the current analysis to finish…'});state.runs.push(run);if(job?.cancelled)queue.unshift({run,input});else queue.push({run,input});persist();return {accepted:true,queued:true,run_id:run.id};
    }
  ...
  // Audience estimates (age, gender, country, state) for saved analyses. Runs as a background job that a submitted link pre-empts.
  const estimated=new Set(),estimateRetry=new Map();
  function needsEstimates(run){
    if(estimated.has(run.id))return false;
    const snapshot=read('analysis-'+run.id,null);if(!snapshot){estimated.add(run.id);return false;}
    const comments=(deps.attachSegments||attachSegments)(snapshot.posts).filter(p=>p.content_kind!=='video'&&p.text);
    const missing=comments.filter(p=>!p.estimate||!('state' in p.estimate)).length;
    if(!missing)estimated.add(run.id);return missing>0;
  }
  async function refreshSegments(runId){if(busy)throw new ServiceError('Collection or sync is already running.',409);const run=select(runId);const snapshot=read('analysis-'+run.id,null);if(!snapshot)throw new ServiceError('Analysis snapshot unavailable.',404);
    busy=true;job={id:run.id,mode:'estimates',stage:'segments',message:'Adding audience estimates (age, gender, state)…',updated_at:now()};
    try{const r=await (deps.segmentPosts||segmentPosts)(snapshot.posts,progress);if(!r.estimated&&r.warnings.length)throw new ServiceError(r.warnings[0],502);
      if(r.estimated>=r.total)estimated.add(run.id);else estimateRetry.set(run.id,clock().getTime()+600000);
      return {estimated:r.estimated,total:r.total,warnings:r.warnings};}
    catch(e){if(!job?.cancelled)estimateRetry.set(run.id,clock().getTime()+600000);throw e;} // a pre-empted backfill resumes on the next idle tick
    finally{busy=false;job=null;persist();const next=queue.shift();if(next)launch(next.run,next.input);}}
  function backfillEstimates(){
    const run=[...state.runs].reverse().find(r=>successful(r)&&!(estimateRetry.get(r.id)>clock().getTime())&&needsEstimates(r));
    if(run)refreshSegments(run.id).catch(()=>{});
    return Boolean(run);
  }
  ...
  function tick(){if(busy||queue.length)return;try{if(state.discovery.enabled&&automaticDue(state.discovery,clock())){start({mode:'discovery'});return;}if(state.live.enabled&&clock()-new Date(state.live.last_at)>=300000){state.live.last_at=now();persist();start({mode:'live',url:state.live.url});return;}
    ...
    if(state.discovery.enabled&&state.discovery.selection?.length&&clock()-lastTrend>=(deps.refreshMinutes??settings.trendingRefreshMinutes)*60000){state.discovery.last_refresh_at=now();persist();start({mode:'trending'});return;}
    if(deps.backfill!==false)backfillEstimates();}catch{/* Explicit job errors appear in status; no background retry storm. */}}
  return {start,view,sync,live,refreshSegments,backfillEstimates,discoveryEnabled,discoveryStatus,importAudience,tick,status:()=>({busy,job,runs:[...state.runs].reverse().map(r=>({...r,label:maskText(r.label)})),live:state.live}),withLock:async(fn)=>{if(busy)throw new ServiceError('Collection or sync running.',409);busy=true;try{return await fn();}finally{busy=false;}}};
}
