...
export function reportPDF(data){return new Promise((resolve,reject)=>{
  const child=spawn(env.ARGUS_PYTHON||'python',['-u',path.join(ROOT,'backend/report.py')],{cwd:ROOT,windowsHide:true,stdio:['pipe','pipe','pipe'],env:{...env,PYTHONIOENCODING:'utf-8',PYTHONPATH:[env.ARGUS_PDF_PACKAGES,env.ARGUS_ML_PACKAGES].filter(Boolean).join(path.delimiter)}});
