...
export function validateEstimate(r) {
  ...
  if (!Number.isFinite(confidence) || confidence < 0.4 || confidence > 1) return {age: null, gender: null, region: null, state: null, confidence: null};
  const region = typeof r.region === 'string' && /^[A-Z]{2}$/.test(r.region.trim().toUpperCase()) ? r.region.trim().toUpperCase() : null;
  const state = region === 'IN' && typeof r.state === 'string' ? INDIAN_STATES.find(s => s.toLowerCase() === r.state.trim().toLowerCase()) || null : null;
  return {
...
export function aggregateEstimates(comments) {
  const known = comments.filter(p => p.estimate), out = {}, stale = known.filter(p => !('state' in p.estimate)).length;
  for (const dim of ['age', 'gender', 'region', 'state']) {
    ...
    out[dim] = {total, unclear: known.length - total, hidden: counts.size - rows.length,
      rows: rows.map(([id, n]) => ({id, label: ['region', 'state'].includes(dim) ? id : ESTIMATES[dim][id], count: n, percent: Math.round(n / total * 1000) / 10}))
  ...
  return {status: known.length ? 'estimated' : 'not_estimated', estimated: known.length, pending: comments.length - known.length + stale, stale,
